...
def codeforces_account_details(link):
    ...
    try:
        ...
        if (page_url.status_code != 200):
            ...
            # highest rating
            highest_rating = soup.find("span", class_="smaller").findChildren()[
                1].contents[0]
            highest_rating = int(highest_rating)

            # current rating
            current_rating = soup.find("div", class_="info").find(
                "ul").findChildren()[2].contents
            ...
            try:
                if (user_id[0] == 'L'):
                    country = soup.find(
                        "div", class_="main-info").findChildren()[8].contents[0]
                    country = str(country)
                else:
                    country = soup.find(
                        "div", class_="main-info").findChildren()[7].contents[0]
                    country = str(country)
            except:
                country = None
            ...
            # email
            try:
                email = soup.find("div", class_="info").find(
                    "ul").findChildren()[11].contents[2]
            ...
            if (email != None):
                current_status = soup.find("div", class_="info").find(
                    "ul").findChildren()[13].find("span").contents[0]
                current_status = str(current_status)
            else:
                current_status = soup.find("div", class_="info").find(
                    "ul").findChildren()[12].contents[0]
                current_status = str(current_status)
            ...
            if (contest_page_url.status_code != 200):
                print(
                    f"Account access request failed ....:{page_url.status_code}")
            else:
                soup2 = BeautifulSoup(contest_page_url.content, "html.parser")
                total_contest = soup2.find(
                    "table", class_="user-contests-table").find("tbody").find("tr").find("td").contents[0]
